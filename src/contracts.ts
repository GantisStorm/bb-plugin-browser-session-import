...
export const rpcContract = {
  ...
  history: {
    input: z
      .object({ destination: z.string().min(1).max(512).optional() })
      .strict(),
    output: z.array(importRecordSchema).max(100),
  },
  importProfile: {
    input: targetSchema
      .extend({
  ...
  clear: {
    input: targetSchema.extend({ confirm: z.literal(true) }).strict(),
    output: z
      .object({ clearedCookies: z.number().int().nonnegative() })
      .strict(),
  },
  openHomepage: {
    input: scopeSchema,
    output: z.object({ tabId: idSchema }).strict(),
  },
  profiles: {
    input: z.object({}).strict(),
    output: z
...
export const requestSchema = z.discriminatedUnion("operation", [
  ...
  z
    ...
    .strict(),
  z
    .object({ operation: z.literal("open-homepage"), target: scopeSchema })
    .strict(),
